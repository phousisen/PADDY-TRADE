...
export default function TypedWeight({ locationId, label, labelKm, hint, value, onChange }) {
  ...
  return (
    <div>
      ...
      {canUseScale ? (
        <button
          type="button"
          onClick={() => { onChange(String(weightKg)); recordCapture(locationId, { by: byRef.current, weightKg: Number(weightKg) }); }}
