...
import { Plus, AlertTriangle, LogOut, KeyRound, Mail, Circle, Pencil, Check, X as XIcon } from "lucide-react";
...
export default function UsersPage() {
  ...
  // renaming someone is a quick one-field fix, not a whole "edit user" flow.
  ...
  async function saveName(u) {
    ...
    if (!trimmed || trimmed === u.full_name) { cancelEditName(); return; }
    setSavingId(u.id);
    try {
      await api.updateProfileRole(u.id, { fullName: trimmed });
      cancelEditName();
      load();
    } catch (err) {
  ...
  // [2026-09-02] The actual on/off switch for an EXISTING account —
  // previously the only way to change this was a direct SQL update, since
  // no UI anywhere exposed it. Same pattern as changeRole/changeLocation
  // above: optimistic-ish (button disables via savingId, then reloads).
  async function toggleViewOnly(u) {
    setSavingId(u.id);
    try {
              ...
              {users.map((u) => {
                ...
                return (
                  <tr key={u.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                    ...
                    <td className="px-3 py-3">
                      {/* [2026-09-02] Access (scope) and the view-only
