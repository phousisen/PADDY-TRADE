...
// [2026-09-03] No more "Use it anyway" — a paper ticket number reused at
// the same station is now blocked outright (see
// add_paper_ticket_no_unique_constraint.sql), so an override here would
// just fail a moment later anyway, either right away if the station is
// online or, worse, silently in the background sync queue if it's not.
// This only ever fires for a genuine duplicate now, so the only real
// action left is going back and fixing the number.
function SanityWarningModal({ warning, onBack }) {
  return (
    <Modal title={`Ticket #${warning.ticketNo} already used`} onClose={onBack}>
      <p className="mb-4 text-sm text-slate-500">
        ...
        {warning.match?.created_at ? ` on ${new Date(warning.match.created_at).toLocaleDateString()}` : ""}. The same number can't be used twice at this station — check the number on the paper slip and correct it before saving.
      </p>
      <div className="flex justify-end gap-2">
        <button onClick={onBack} className="rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700">
          Go back and check
        </button>
      </div>
    </Modal>
...
function NewTicketModal({ locations, defaultLocationId, isAdmin, onClose, onCreated, initialType }) {
  ...
  async function submit() {
    ...
    if (trimmedTicketNo) {
      ...
      if (!dupMatch) {
        // [2026-09-04] Normalized the same way (squeeze internal
        // whitespace, not just trim the ends) — see normalizePaperTicketNo
...
function FinishTicketModal({ ticket, onClose, onFinalized, onDeclined, isAdmin }) {
  ...
  const rawNetKg = isBuy
    ? (ticket.gross_kg || 0) - (parseFloat(tareWeight) || 0)
    : (parseFloat(tareWeight) || 0) - (ticket.gross_kg || 0);
  const netKg = Math.max(0, rawNetKg);
  // [2026-09-12] THE CLAMP ABOVE WAS SILENT, AND SIX REAL TRUCKLOADS WENT
  // THROUGH IT.
  //
  // The direction of the subtraction has always been right here. What was
  // missing is any reaction to it coming out negative or zero: Math.max
  // turned it into 0, submitFinish only ever checked that the weigh-out box
  // had SOME number in it, and Finish Ticket went ahead and created a real
  // transaction of 0 kg for 0 ៛ — a truck that physically arrived, was
  // weighed twice and left, recorded as nothing. Found 2026-09-12 by
  // running find_zero_weight_sells_2026-09-12.sql: six tickets between 23
  // and 29 August, across PONG RO, PING PONG and JOMNOUM, including one
  // where the truck gained 23,725 kg and one where it gained 34,140 kg.
  //
  // Two different mistakes produce it, and this cannot tell them apart —
  // which is exactly why it must stop and ask a person:
  //   * the two weights went into the wrong boxes, or
  //   * a SALE was started as a BUY ticket (a truck that GAINS weight is
  //     being loaded, not unloaded) — and that one cannot be fixed later
  //     from the Transactions screen, because a ticket's type cannot be
  //     changed after the fact.
  const bothWeighed = ticket.gross_kg != null && String(tareWeight).trim() !== "";
  const weightsReversed = bothWeighed && rawNetKg <= 0;
  const reversedMessage = isBuy
    ? "On a purchase the truck arrives LOADED and leaves EMPTY, so Weigh In must be more than Weigh Out. This truck got heavier — either the two weights are in the wrong boxes, or this is actually a SALE started as a Buy ticket."
    : "On a sale the truck arrives EMPTY and leaves LOADED, so Weigh Out must be more than Weigh In. This truck got lighter — either the two weights are in the wrong boxes, or this is actually a PURCHASE started as a Sell ticket.";
  ...
  async function submitFinish() {
    ...
    // [2026-09-12] See the comment on weightsReversed above. A ticket whose
    // two weighs do not make a load in this ticket's own direction must
    // never become a 0 kg transaction — it has to be put right, or declined,
    // by someone who can see the truck and the paper.
    if (weightsReversed) { setError(reversedMessage); return; }
      ...
      {weightsReversed && (
        <div className="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-4">
          <p className="text-sm font-semibold text-rose-800">
            These two weights do not make a load.
            <span className="font-khmer block font-normal">ទម្ងន់ទាំងពីរនេះមិនត្រឹមត្រូវទេ</span>
          </p>
          <p className="mt-1.5 text-[12px] leading-relaxed text-rose-700">{reversedMessage}</p>
          <p className="mt-1.5 text-[12px] leading-relaxed text-rose-700">
            Weigh In <b>{fmt2(ticket.gross_kg)} kg</b> · Weigh Out <b>{fmt2(parseFloat(tareWeight) || 0)} kg</b>
            {" — "}a difference of <b>{fmt2(Math.abs(rawNetKg))} kg</b> the wrong way.
            This ticket cannot be finished until it is right. Re-capture the weight, or Decline it.
          </p>
        </div>
      )}
      <div className="mt-4 space-y-1.5 rounded-lg bg-slate-50 p-4 text-sm">
        <div className="flex justify-between"><span className="text-slate-500">Net Weight <span className="font-khmer">ទម្ងន់សុទ្ធ</span></span><span className={`font-medium ${weightsReversed ? "text-rose-600" : ""}`}>{fmt2(netKg)} kg</span></div>
